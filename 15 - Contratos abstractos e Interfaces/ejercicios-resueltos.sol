...
interface IVehiculo{
    ...
    function detener () external;
}
    
abstract contract BaseVehiculo is IVehiculo {
    uint public combustible;
    bool public enMovimiento;

    function detener() external override {
        enMovimiento = false;
    }

    function cargarCombustible(uint _cantidad)internal{
        combustible += _cantidad; 
    }

}

contract Auto is BaseVehiculo{
    constructor(){
        enMovimiento = false;
        combustible = 100;
    }
    ...
    function acelerar() external override {
        require(combustible >= 10, "Sin nafta!");
        enMovimiento = true;
        combustible -= 10;
    }
}